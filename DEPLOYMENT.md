...
> **Production access is gated separately, and it's the long pole.** Completing the App content checklist does not unlock Production — personal Play Console accounts require a closed test with 12+ testers opted in continuously for 14+ days before you can even apply for production access, then a Google review on top of that. This has nothing to do with `eas build`/`eas submit` and can't be skipped by them. See `deployment/README.md` §7 for the exact steps — start it early, since the 14-day clock is the real bottleneck in shipping this app, not the build/submit steps below.
